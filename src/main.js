...
app.put("/products/:pid", async (req, res) => {
    ...
    try {
        const productReemplazada = await um.actualizarProductoId(
...
io.on("connection", (clientSocket) => {
    console.log(`nuevo usuario ${clientSocket.id}`);

  io.sockets.emit('actualizarProdcutos', realTimeProducts )
});
   

app.get("/", async (req, res) => {
    const realTimeProducts = await um.buscarProduct();
    const productsParaFront = realTimeProducts
        .map(
            (product) => `
    <li>
